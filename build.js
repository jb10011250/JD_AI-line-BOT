...
`;
  ...
  (async () => {
    ...
    if (fs.existsSync(kbMdDir) && fs.readdirSync(kbMdDir).some(f => f.endsWith('.md'))) {
      ...
      // ⚠️ 備援方案：讀取原始 DOCX
      console.log("⚠️  找不到 knowledge_md，改用 knowledge/*.docx 備援...");
      const docxFiles = fs.readdirSync(kbDocxDir).filter(f => f.endsWith('.docx') || f.endsWith('.txt'));
      const results = await Promise.all(docxFiles.map(async (file) => {
        const filePath = path.join(kbDocxDir, file);
        try {
